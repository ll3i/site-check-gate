...
def verify_item(item, offline=False):
    ...
    if doi:
        ...
        if not registered:
            ...
            if not candidates:
                # DOI 미등록 + 검색 후보 없음 → 상태 결정 없이 케이스3(검색)으로 진행
                # (아래 케이스3에서 근거 없으면 ❌ + 'DOI 미등록' 비고)
                result['note'] = f"DOI {doi} 는 doi.org 에 미등록."
                # 여기서 return하지 않고 케이스3로 계속 진행
            ...
            if best and title_similarity(title, best.get('title', '')) >= 0.72:
                ...
                result['candidates'] = candidates[:3]
                return result  # status 없음 → ❌ 판정 로직에서 처리
        ...
        if sim >= 0.85 and (yr_diff is None or yr_diff <= 1) and (not source_author or author_match(input_author, source_author)):
            ...
            result['diffs'] = diffs
            result['note'] = "확인 완료: DOI 실존 + 제목 유사도 {:.2f} + 연도차 {}년".format(sim, yr_diff if yr_diff else 0)
            return result
        else:
            result['status'] = 'partial'
            result['matched'] = matched
...
def main():
    ...
    for item in items:
        ...
        print(f"  [{result['id']}] status={result['status']} note={result.get('note','')[:60]}")
